...
function Nav() {
  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-auto">
      <nav className="flex items-center gap-1 sm:gap-2 bg-[#18181b] text-neutral-300 p-1.5 px-2 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-neutral-800 backdrop-blur-md">
        {/* Logo / Planet Icon */}
        <NavLink
          to="/"
        ...
        <div className="flex items-center gap-1">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `text-sm font-medium px-4 py-1.5 rounded-full transition-all duration-200 no-underline ${
                  isActive
